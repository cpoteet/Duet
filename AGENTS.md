...
- Build with `./build.sh`. It compiles using the local `swiftc` toolchain, assembles `dist/Duet.app`, and ad-hoc signs it.
- Test with `./test.sh`.
...
- Support provider-managed sign-in, passkeys, 2FA, and popup flows. Never collect or store user credentials.
